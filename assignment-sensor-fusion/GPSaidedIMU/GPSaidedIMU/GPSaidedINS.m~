...
for k=2:N
    ...
    g = in_data.GNSS.pos_ned(:,ctr_gnss_data);
...
%---> Change needed for tasks 2-4, make sure you understand it...
     ...
     % Check if GNSS measurement is available
     if t(k)==in_data.GNSS.t(ctr_gnss_data) 
       fprintf('%d %d %d %d: we got into here!\n', ctr_gnss_data, t(k), settings.outagestart, strcmp(settings.gnss_outage,'on'))
       if ctr_gnss_data > settings.outagestart && ctr_gnss_data < settings.outagestop && strcmp(settings.gnss_outage,'on')
           fprintf('change ind state\n')
           ind(1:3)=0;
       else
           fprintf('def ind state\n')
           ind(1:3) = 1;
        % Update GNSS data counter
       end
...
%--->    
    ...
    x_h(1:6)=x_h(1:6)+z(1:6);                   %update position and velocity
    x_h(7:10)=Gamma(x_h(7:10),z(7:9));          %change the quaternion attitude
    delta_u_h=z(10:15);
